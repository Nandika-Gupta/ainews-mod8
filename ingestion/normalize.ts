/**
 * URL/date/text normalization utilities. Ported and reworked from GraphOne's
 * atlas_ingestion/normalization.py (normalize_url) and publish_to_backend.py
 * (robust_parse_date, clean_text) — same techniques, rewritten in TypeScript
 * and trimmed to what a news pipeline actually needs.
 */
 ...
 * slash. This is the value stored as NewsArticle.articleUrl and is what
 * makes duplicate-detection across crawls reliable.
 */
...
/**
 * Jaccard similarity between two token sets — a cheap stand-in for the
 * fuzzy title-matching GraphOne does with difflib.SequenceMatcher, used to
 * catch the same story reported by two different publishers under slightly
 * different headlines (different articleUrl, so exact-URL dedup misses it).
 */
