/**
 * Publisher (news OUTLET) resolution and creation.
 *
 * This is the direct rewrite of the Slack-flagged GraphOne behavior: GraphOne
 * sets `isOfficial: true` and attaches the SUBJECT COMPANY's logo (see
 * publish_to_backend.py::submit_news, `"name": f"{company} News"`). Our
 * module inverts that completely — the Publisher is whoever wrote/published
 * the article, and its logo is resolved and cached ONCE here, then reused by
 * every future article from that domain. The company mentioned in an
 * article's title is never a logo source.
 */
...
/**
 * Resolves a publisher's logo exactly once, at creation time:
 *   1. A bundled brand SVG already in /public/logos, if this is a known source.
 *   2. Otherwise, the real Publisher Resolver (logoResolver.ts): fetch the
 *      domain's homepage, parse <head> icon links + manifest, download the
 *      highest-resolution one, and store it under public/logos/publishers/.
 *   3. Only if that download genuinely fails (network error, no icons found)
 *      does this fall back to a live favicon/aggregator URL, so publisher
 *      creation never hard-fails.
 * The result is stored on the Publisher row — never re-resolved per article.
 */
async function resolveLogo(domain: string, sourceKey?: string): Promise<{ logoUrl: string; faviconUrl: string }> {
  const faviconUrl = `https://${domain}/favicon.ico`;

  const bundled = sourceKey ? SOURCE_LOGOS[sourceKey] : undefined;
  if (bundled) return { logoUrl: `/logos/${bundled}`, faviconUrl };

  const downloaded = await resolvePublisherLogo(domain);
 ...
 * already know about.
 */
export async function resolvePublisher(domain: string, nameHint?: string | null) {
  const existing = await prisma.publisher.findUnique({ where: { domain } });
  if (existing) return existing;

  const known = KNOWN_BY_DOMAIN[domain];
  const { logoUrl, faviconUrl } = await resolveLogo(domain, known?.sourceKey);
